...
def fisher_jenks(values, classes=5, cores=None, sort=True):
	...
	sharedVar[i] = numpy.apply_along_axis(calcVar, 1, sharedVar[i], len(values))
	if start > 1000:
	    print sharedVar[2499]
    ...
    print "Allocation Complete using HDF5"
    ...
    del jobs[:], p, job
    t1 = time.time()
    print "Variance Matrix Complete"
	...
	stdout.write("\rFinished Row {} / {}.".format(row, len(sharedErr[:,1]-1)))
	errRow = x[row:]
