        ...
        for polys in checks:
            ...
            for k, v in vertices.iteritems():
                ...
                for neigh in potential_neigh:
                    ...
                    if len(xintersects) > 1 and len(yintersects) > 1:
                        #We have two shared points - are they adjacent in the poly geom, i.e. an edge?
                        #Since the intersect is on x and y, we are sage to just index chcek x?
    ...
    for fname in fnames:
        ...
        for i in xrange(1, len(xv[0])):
            ...
            for j in xrange(1, len(yv[:,0])):
                ...
                #Look for the x,y spanners
                rollbackidx = np.where((sortmem[:,0] != sortmem[:,1]) & (sortmem[:,2] != sortmem[:,3]))[0]
                if len(sortmem[rollbackidx, -1]) != 0:
                    stepback[sortmem[rollbackidx, -1] = j
        ...
        w = {}
        print np.where(npcontmatrix[0] != 0)
        print np.where(npcontmatrix[:,0] != 0)
        exit()
        tb = time.time()
